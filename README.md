...
Field accuracy says how well an invoice was *read*, not whether the audit
engine *flagged* the right invoices. That is measured separately:
...
[docs/audit-eval.md](docs/audit-eval.md). A class in the ground truth but
...
| Real extraction, auditable subset | 0.733 | 0.917 | 0.815 |
| Real extraction, failed extractions counted as missed | 0.733 | 0.917 | 0.815 |

Field-level accuracy on the same runs: **0.941** over the whole batch with failed
extractions counted as empty documents (the convention of the recorded 0.8249
baseline — beaten by **+11.4 pp**), 0.942 on the auditable subset — which in the
first run was a *different population*, because that run excluded the 3–4
documents per round it could not read.

**The measured 12.2% extraction-failure rate was then fixed and re-measured.**
The first run of this page reported 11 failures in 90 attempts (12.2%), every one
the same defect: `qwen-vl-plus` returned JSON whose outermost object was never
closed, or whose tail was a degenerate repetition that the naive
`first "{"/last "}"` slice glued into something no parser accepts. The extractor
now closes a truncated object, takes the first complete object when the model
echoes a second one, and keeps the **first** occurrence of a repeated key — the
echo is degeneration, and last-key-wins had let the model's placeholder skeleton
(`"金额": 0`) overwrite a number it had already read correctly. A response that is
still unusable gets one corrective re-read carrying its own output and the
parser's complaint.

Re-running the same 30 invoices × 3 rounds: **failure rate 12.2% → 0.0% (0/90)**,
micro F1 **0.676 → 0.815**, recall **0.694 → 0.917**, precision 0.658 → 0.733, at
91 API calls instead of 117. All three rounds now score *identically*, because the
largest source of variance — which invoices happened to fail that round — is
gone; what remains is OCR error alone (one masked arithmetic anomaly and four
false positives per round, the same four every time). The before/after table is
generated by the report itself from a recorded baseline, so the claim is
checkable rather than remembered — see [docs/e2e-eval.md](docs/e2e-eval.md).
...
- **React + TypeScript + Vite 看板（浅色主题，中英双语）**：右上角 EN/中文
  一键切换（localStorage 记忆）；单张/批量上传、处理进度、字段+置信度表格、
  按级别着色的告警面板、图谱视图、CSV/JSON 导出、失败文件单独重试；界面
  明确区分"真实上传"与"合成演示数据"；
...
- **端到端评测（真实视觉模型 → 抽取 → 审计 → 打分）**：把 30 张 PDF 真实
  ...
  `docs/e2e-eval.md`）。第一轮实测出 12.2% 的抽取失败率（90 次里 11 次，
  全部是模型返回的 JSON 外层花括号未闭合/尾部退化重复），**修掉后重测**：
  失败率 12.2% → 0.0%，审计 micro F1 0.676 → 0.815、召回 0.694 → 0.917
  （标注字段上的 1.000 是上界，真实链路才是可交付数字）。残余损失已全部
  来自 OCR 本身：某张票的算术异常被"自我一致地读错"从而被掩盖（轮轮复现），
  另有多张正常票因金额/号码误读被 `qr_crosscheck`、`party_info` 误报；
